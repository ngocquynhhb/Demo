...
public class SpawnDrone : MonoBehaviour, IDataPresistent
{
    public ObjectPooling objectPool;
    public Transform playerTransform;
    public float minDistanceBetweenEnemies = 3f;
    public float maxDistanceBetweenEnemies = 20f;
    public int totalNumberOfEnemies = 5;
    public float yPosition = -0.38f;
    ...
    private void Start()
    {
        if (gameData.dronePositions.Count != 0)
        {
            return;
        }
    ...
    private List<Transform> GenerateRandomSpawnPositions(int numberOfPositions, float startPosition, float distance)
    {
        ...
        for (int i = 0; i < numberOfPositions; i++)
        {
            ...
            spawnPositionObject.transform.position = new Vector3((startPosition + i * distance) + 15, yPosition, 0f);
    ...
    public void SaveData(ref GameData data)
    {
        ...
        {
            data.dronePositions.Add(enemy.transform.position);
        }
    }
    ...
    public void LoadData(GameData data)
    {
        ...
        int totalEnemies = data.dronePositions.Count;
        for (int i = 0; i < totalEnemies; i++)
        {
            Vector3 enemyPosition = data.dronePositions[i];
